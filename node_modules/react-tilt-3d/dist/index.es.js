...
const calculateTiltOnAxis = (axis, position, domNode, options) => {
  if (options.lockAxis)
    return { degrees: 0, exceedsLimit: false };
  ...
  const tilt = distanceFromCenter / distanceAtMaxTilt * options.maxTilt * (axis === "y" ? -1 : 1);
  const exceedsLimit = Math.abs(distanceFromCenter) > Math.abs(distanceAtMaxTilt);
  if (exceedsLimit) {
...
const getDistanceFromEdge = (elem) => {
...
const calculateTilt = (cursorPos, domNode, options) => {
  if (!cursorPos.x && !cursorPos.y || options.resetTiltOnHover && domNode.matches(":hover"))
    return { x: 0, y: 0 };
...
const BOUNCE_TRANISTION = "cubic-bezier(.32,.66,.72,1.58)";
const defaultParams = {
  zoomOnTilt: false,
  zoomScale: 1,
  transition: "ease-out",
...
const generateStyleText = (params) => {
  const {
    tilt,
    maxTilt,
    zoomOnTilt,
  ...
  const brightness = !shouldApplyLighting ? 1 : Math.round((tilt.y / maxTilt + 1) * 100) / 100;
  ...
  const transitionAnimation = transition === "bounce" ? BOUNCE_TRANISTION : transition;
  const style = {
  ...
  const styleText = Object.entries(style).reduce((acc, [key, value]) => `${acc} ${key}: ${value};`, "").trim();
  return styleText;
};
const getCurrentTilt = (domNode) => {
  const style = domNode.style.transform;
  const getRotationDegrees = (axis) => {
    var _a;
    return Number((_a = style.match(new RegExp(`${axis}\\((.*?)deg\\)`))) == null ? void 0 : _a[1]) || 0;
  };
...
const REGISTRATION_THRESHOLD = 1e3;
let lastOrientationRegistered = {
  beta: null,
  gamma: null,
  registeredAt: null
};
const THRESHOLD = 3;
const isWithinThreshold = (a, b) => Math.abs(Math.abs(a) - Math.abs(b)) < THRESHOLD;
const saveInitialOrientation = ({ beta, gamma }) => {
  ...
  if (Object.values(initialOrientation).some((angle) => angle === null) || Object.values(initialOrientation).every((angle) => angle === 0)) {
    setInitialOrientation();
    registerLastOrientation();
  }
  const now = Date.now();
  const mustRegisterAgain = now - lastOrientationRegistered.registeredAt > REGISTRATION_THRESHOLD;
  const stoppedMoving = mustRegisterAgain && isWithinThreshold(lastOrientationRegistered.beta, beta) && isWithinThreshold(lastOrientationRegistered.gamma, gamma);
  if (stoppedMoving) {
    setInitialOrientation();
  }
...
const calculateTiltFromGyro = ({ beta, gamma }, maxTilt) => {
  if ([beta, gamma].some((angle) => angle === null)) {
    return { x: 0, y: 0 };
  }
  ...
  useEffect(() => {
    const resetTilt = () => {
      if (ref.current) {
        ...
        if (currentTilt.isTilted) {
          onTiltEnd == null ? void 0 : onTiltEnd();
        }
    ...
    const applyTiltFromGyro = (e) => {
      if (ref.current) {
        ...
        onTiltChange == null ? void 0 : onTiltChange(tilt);
      }
    };
    const applyTilt = ({ x, y }) => {
      if (ref.current) {
        ...
        if (tiltStarts)
          onTiltStart == null ? void 0 : onTiltStart();
        if (tiltEnds)
          onTiltEnd == null ? void 0 : onTiltEnd();
        const newStyle = generateStyleText({
          tilt,
...
async function requestGyroPermission() {
  const deviceMotion = DeviceOrientationEvent;
  if (typeof DeviceOrientationEvent === "undefined" || !deviceMotion.requestPermission) {
